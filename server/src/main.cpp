...
CConfig::CConfig()
{
	...
	str_copy(m_aWebDir, "/mnt/sda/maqingyao/ServerStatus-Hotaru/ServerStatus/web", sizeof(m_aWebDir)); // -d, --web-dir
	str_copy(m_aTemplateFile, "template.html", sizeof(m_aTemplateFile));
	str_copy(m_aJSONFile, "/hotaru_theme/dist/json/stats.json", sizeof(m_aJSONFile));
...
int CMain::HandleMessage(int ClientNetID, char *pMessage)
{
	...
	if(str_comp_num(pMessage, "update", sizeof("update")-1) == 0)
	{
		...
		//add gpu
		if(rStart["gpu_utilization"].type)
			pClient->m_Stats.gpu_utilization = rStart["gpu_utilization"].u.integer;
		if(rStart["gpu_memory_used"].type)
			pClient->m_Stats.gpu_used = rStart["gpu_memory_used"].u.integer;
		if(rStart["gpu_memory_total"].type)
			pClient->m_Stats.gpu_total = rStart["gpu_memory_total"].u.integer;
