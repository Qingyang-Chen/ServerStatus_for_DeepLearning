# -*- coding: utf-8 -*-
# Support Python Version 2.7 to 3.9
...
def get_nvidia():
    info = subprocess.check_output(
        ['nvidia-smi', '--query-gpu=memory.used,memory.total,utilization.gpu', '--format=csv']).decode('utf-8')
    p = info.splitlines()[-1]
    utilization = int(p.split(',')[-1].split('%')[0])
    memory_total = int(p.split(',')[-2].split('MiB')[0])
    memory_used = int(p.split(',')[-3].split('MiB')[0])
    ...
    while True:
        try:
            ...
            while True:
                ...
                #add gpu
                GpuUtilization, GpuMemoryUsed,  GpuMemoryTotal= get_nvidia()
                ...
                #add gpu
                array['gpu_utilization'] = GpuUtilization
                array['gpu_memory_used']= GpuMemoryUsed
                array['gpu_memory_total'] = GpuMemoryTotal
                #print(array)
